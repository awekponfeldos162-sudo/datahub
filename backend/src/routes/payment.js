...
// Plans tarifaires — accessible sans connexion (page pricing publique)
...
// Webhook Flutterwave — appelé directement par les serveurs Flutterwave (pas de JWT)
router.post('/webhook', express.raw({ type: 'application/json' }), async (req, res, next) => {
  try {
    const secretHash = process.env.FLUTTERWAVE_WEBHOOK_SECRET;
    const signature = req.headers['verif-hash'];

    // Vérification de la signature Flutterwave — seule défense contre les faux webhooks
...
// Webhook CinetPay — appelé directement par les serveurs CinetPay (pas de JWT)
router.post('/webhook-cinetpay', express.json(), async (req, res, next) => {
  try {
    ...
    // Vérification du site ID CinetPay — seule défense contre les faux webhooks
...
router.post('/initialize', authenticate, validate(schemas.initPayment, 'body', true), async (req, res, next) => {
  try {
...
router.post('/initialize-cinetpay', authenticate, validate(schemas.initPayment, 'body', true), async (req, res, next) => {
  try {
