...
export default function Heatmap() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-bold text-slate-900">Heatmap d'engagement</h2>
      ...
      <div className="card p-4 flex flex-wrap gap-4 items-center">
        <div>
          <label className="label text-xs mb-1">Plateforme</label>
          <select className="input py-1.5 text-sm" value={platform} onChange={(e) => setPlatform(e.target.value)}>
            {PLATFORMS.map((p) => (
        ...
        <div>
          <label className="label text-xs mb-1">Métrique</label>
          <select className="input py-1.5 text-sm" value={metric} onChange={(e) => setMetric(e.target.value)}>
            {METRICS.map((m) => (
      ...
      {bestSlot && (
        <div className="card p-4 bg-primary-50 border-primary-200 flex items-start gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-primary-900">Meilleur créneau détecté</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="card p-5">
          <h4 className="text-sm font-semibold text-slate-900 mb-3">Top 5 heures</h4>
          <div className="space-y-2">
            {[...heatmapData]
              .sort((a, b) => b.value - a.value)
              .slice(0, 5)
              .map((d, i) => {
                const max = heatmapData[0]?.value || 1;
        ...
        <div className="card p-5">
          <h4 className="text-sm font-semibold text-slate-900 mb-3">Engagement par jour</h4>
          <div className="space-y-2">
            {DAYS_FULL.map((day, dayIdx) => {
              ...
              return (
                <div key={day} className="flex items-center gap-3">
                  <span className="text-xs text-slate-500 w-16 shrink-0">{day.slice(0, 3)}</span>
                  <div className="flex-1 bg-slate-100 rounded-full h-2">
