...
export default function Analytics() {
  ...
  const { mutate: sync, isLoading: syncing } = {
    mutate: async () => {
      try {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card p-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="font-bold text-lg text-slate-900">{platform.toUpperCase()}</h2>
              <p className="text-slate-500 text-sm">@{account?.username || 'Non connecté'}</p>
              <div className="flex items-center gap-2 mt-1">
                <span className="badge bg-slate-100 text-slate-700">{(account?.followers || 0).toLocaleString()} abonnés</span>
                {account?.lastSync && (
      ...
      {chartData.length > 0 && (
        <div className="card p-6">
          <h3 className="text-base font-semibold text-slate-900 mb-4">Performance des publications</h3>
      ...
      <div className="card">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <h3 className="text-base font-semibold text-slate-900">Publications ({pagination?.total || 0})</h3>
          <select
            value={sortBy}
          ...
          {(posts || []).map((post) => (
            <div key={post.id} className="flex items-center gap-4 px-6 py-4 hover:bg-slate-50 transition-colors">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-slate-900 truncate">{post.title || 'Sans titre'}</p>
                <div className="flex items-center gap-3 mt-1">
                  <span className="badge bg-slate-100 text-slate-600 text-[10px]">{post.type}</span>
                  <span className="text-xs text-slate-400">
                    {new Date(post.publishedAt).toLocaleDateString('fr-FR')}
                  </span>
                </div>
              </div>
              <div className="hidden md:flex items-center gap-4 text-sm text-slate-600 shrink-0">
                <span className="flex items-center gap-1"><Eye size={12} /> {(post.totals?.views || 0).toLocaleString()}</span>
                <span className="flex items-center gap-1"><Heart size={12} /> {(post.totals?.likes || 0).toLocaleString()}</span>
