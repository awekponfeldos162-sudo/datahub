    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
        {data.map((platform) => (
          <div key={platform.platform} className="card p-4 border-t-4" style={{ borderTopColor: PLATFORM_COLORS[platform.platform] }}>
            <h3 className="font-bold text-sm text-slate-900">{platform.platform}</h3>
            <p className="text-xs text-slate-500 mb-3">@{platform.username || 'N/A'}</p>
            <div className="space-y-1.5">
              <div className="flex justify-between text-xs">
                <span className="text-slate-500">Abonnés</span>
                <span className="font-medium">{(platform.followers || 0).toLocaleString()}</span>
              </div>
              <div className="flex justify-between text-xs">
                <span className="text-slate-500">Vues</span>
                <span className="font-medium">{parseInt(platform.views || 0).toLocaleString()}</span>
              </div>
              <div className="flex justify-between text-xs">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        <div className="card p-6">
          <h3 className="text-base font-semibold text-slate-900 mb-4">Métriques comparées</h3>
        ...
        {/* Radar chart */}
        <div className="card p-6">
          <h3 className="text-base font-semibold text-slate-900 mb-4">Score global par plateforme</h3>
          <ResponsiveContainer width="100%" height={260}>
      ...
      <div className="card">
        <div className="px-6 py-4 border-b border-slate-100">
          <h3 className="text-base font-semibold text-slate-900">Classement des plateformes</h3>
        </div>
        <table className="w-full text-sm">
          <thead>
            <tr className="text-left border-b border-slate-100">
              {['#', 'Plateforme', 'Abonnés', 'Publications', 'Vues', 'Engagement %', 'Partages'].map((h) => (
                <th key={h} className="px-6 py-3 font-medium text-slate-500 text-xs">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-50">
            {[...data].sort((a, b) => b.views - a.views).map((p, i) => (
              <tr key={p.platform} className="hover:bg-slate-50">
                ...
                <td className="px-6 py-3">{(p.followers || 0).toLocaleString()}</td>
                <td className="px-6 py-3">{p.posts || 0}</td>
                <td className="px-6 py-3">{parseInt(p.views || 0).toLocaleString()}</td>
                <td className="px-6 py-3">
                  <span className={`badge ${parseFloat(p.engagementRate) >= 3 ? 'bg-green-50 text-green-700' : 'bg-yellow-50 text-yellow-700'}`}>
                    {p.engagementRate}%
                  </span>
                </td>
                <td className="px-6 py-3">{parseInt(p.shares || 0).toLocaleString()}</td>
              </tr>
            ))}
          </tbody>
        </table>
